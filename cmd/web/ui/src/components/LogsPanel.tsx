...
export default function LogsPanel() {
  ...
  const colorLine = (line: string) => {
    ...
    if (line.includes('[INFO]'))  return 'text-zinc-700 dark:text-zinc-300'
    if (line.includes('[DEBUG]')) return 'text-zinc-400 dark:text-zinc-500'
    return 'text-zinc-500 dark:text-zinc-400'
  }

  return (
    <div className="flex-shrink-0 bg-zinc-50 dark:bg-zinc-900 border-t border-zinc-200 dark:border-zinc-800">
      <div className="flex items-center justify-between px-4 py-1 border-b border-zinc-200 dark:border-zinc-800">
        <div className="flex items-center gap-3">
          ...
          <span className="text-zinc-400 dark:text-zinc-600 text-xs">{lines.length} lines</span>
        </div>
        {open && (
          <div className="flex items-center gap-3">
      ...
      {open && (
        <div className="h-48 overflow-y-auto px-4 py-2 text-xs leading-relaxed">
          {lines.length === 0 ? (
            <p className="text-zinc-400 dark:text-zinc-600 italic">No log output yet…</p>
          ) : (
            lines.map((line, i) => (
