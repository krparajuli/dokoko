...
func (m model) handleMenuKey(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case "3": // Prune Cache — no form, enqueues immediately
		m.activeOp = 3
		_, err := m.actor.PruneCache(context.Background(), dockertypes.BuildCachePruneOptions{})
		if err != nil {
			m.resultText = "Error: " + err.Error()
		} else {
			m.resultText = "Prune cache queued.\n\nThe operation runs in the background.\nCheck the Live State pane for results."
		}
		m.leftView = viewResult
		return m, nil
...
// submitForm validates inputs and dispatches the active operation.
func (m model) submitForm() (tea.Model, tea.Cmd) {
	...
	var req dockerbuildops.BuildRequest
	// vals: [0]=contextDir|url, [1]=tags, [2]=dockerfile, [3]=target
	tags := parseTags(vals[1])
	dockerfile := vals[2]
	target := vals[3]

	switch m.activeOp {
	case 1: // Build from local directory
		req = dockerbuildops.BuildRequest{
			ContextDir: vals[0],
			Tags:       tags,
			Dockerfile: dockerfile,
			Target:     target,
		}
	case 2: // Build from remote URL
		req = dockerbuildops.BuildRequest{
			RemoteContext: vals[0],
			Tags:          tags,
			Dockerfile:    dockerfile,
			Target:        target,
		}
	...
	_, err := m.actor.Build(context.Background(), req)
	if err != nil {
		m.formErr = err.Error()
		return m, nil
	}
	...
	m.resultText = fmt.Sprintf(
		"Build queued: %s\n\nWatch the Live State pane for progress.\nThe build will appear in Active when complete.\n\nBuilds can take several minutes — the pane\nupdates automatically every 300 ms.",
		tagSummary,
	)
	m.leftView = viewResult
	return m, nil
...
func (m model) renderMenu() string {
	...
	sb.WriteString(titleStyle.Render("Docker Build Manager") + "\n\n")
...
func (m model) renderForm() string {
	...
	sb.WriteString(dimStyle.Render("[Enter] next/submit  [Tab] cycle  [Esc] back"))
	return sb.String()
}
