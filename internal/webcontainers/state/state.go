// Package webcontainersstate manages the in-memory lifecycle of web-container
// provisioning and termination requests.
//
// Lifecycle:
//
...
// Operation is the kind of web-container action being requested.
...
type StateChange struct {
	ID          string            // unique, sortable: "wcchg-<nano>-<seq>"
	Op          Operation         // what to do
	Ref         string            // user ID
	Meta        map[string]string // optional metadata (catalog_id, etc.)
...
type ActiveRecord struct {
	Change      *StateChange
	ResultRef   string // e.g. "containerID:hostPort"
...
// State is the single source of truth for web-container state changes.
...
func New(log *logger.Logger) *State {
	log.LowTrace("initialising webcontainer state store")
	s := &State{log: log}
	log.Info("webcontainer state store ready")
	return s
}
...
func (s *State) RequestChange(op Operation, userID string, meta map[string]string) *StateChange {
	s.log.LowTrace("webcontainer: requesting state change: op=%s ref=%s", op, userID)

	seq := atomic.AddUint64(&s.seq, 1)
	id := fmt.Sprintf("wcchg-%d-%06d", time.Now().UnixNano(), seq)
	...
	s.log.Info("webcontainer: state change requested: id=%s op=%s ref=%s (total=%d)", id, op, userID, n)
	return change
}
...
func (s *State) ConfirmSuccess(changeID, resultRef string) (*ActiveRecord, error) {
	...
	s.log.Info("webcontainer: state change confirmed: id=%s op=%s ref=%s", changeID, change.Op, change.Ref)
	return rec, nil
}
...
func (s *State) RecordFailure(changeID string, err error) (*FailedRecord, error) {
	...
	s.log.Warn("webcontainer: state change failed: id=%s op=%s ref=%s err=%v", changeID, change.Op, change.Ref, err)
	return rec, nil
}
...
func (s *State) Abandon(changeID, reason string) (*AbandonedRecord, error) {
	...
	s.log.Warn("webcontainer: state change abandoned: id=%s op=%s ref=%s reason=%q", changeID, change.Op, change.Ref, reason)
	return rec, nil
}
...
var ErrChangeNotFound = errors.New("webcontainer state change not found")
