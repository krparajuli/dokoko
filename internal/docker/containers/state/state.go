// Package dockercontainerstate manages the in-memory lifecycle of Docker
// container state change requests for this program.
//
// Lifecycle:
//
...
// Operation is the kind of container action being requested.
...
type StateChange struct {
	...
	ContainerRef string            // name or ID, e.g. "my-container" or "a1b2c3d4e5f6"
	Meta         map[string]string // optional: image, force, timeout, etc.
...
type ActiveRecord struct {
	Change      *StateChange
	DockerID    string // container ID returned by the daemon (empty for stops/removes)
...
// State is the single source of truth for container state changes in this
...
func New(log *logger.Logger) *State {
	log.LowTrace("initialising container state store")
	s := &State{log: log}
	log.Debug("state store allocated (requested=0 active=0 failed=0 abandoned=0)")
	log.Info("container state store ready")
	return s
}
...
func (s *State) RequestChange(op Operation, containerRef string, meta map[string]string) *StateChange {
	s.log.LowTrace("requesting state change: op=%s ref=%s", op, containerRef)
	...
	s.log.Info("state change requested: id=%s op=%s ref=%s", id, op, containerRef)
	return change
}

// ConfirmSuccess moves the change identified by changeID from the requested
// bucket into the active bucket.  dockerID is the container ID reported by
// the daemon (may be empty for stop/remove operations).
//
...
func (s *State) ConfirmSuccess(changeID, dockerID string) (*ActiveRecord, error) {
	...
	s.log.Debug("found change at requested[%d]: op=%s ref=%s", idx, change.Op, change.ContainerRef)
	...
	s.log.Info("state change confirmed active: id=%s op=%s ref=%s dockerID=%s",
		changeID, change.Op, change.ContainerRef, dockerID)
	return rec, nil
}
...
func (s *State) RecordFailure(changeID string, err error) (*FailedRecord, error) {
	...
	s.log.Warn("state change failed: id=%s op=%s ref=%s err=%s",
		changeID, change.Op, change.ContainerRef, err)
	return rec, nil
}
...
func (s *State) Abandon(changeID, reason string) (*AbandonedRecord, error) {
	...
	s.log.Warn("state change abandoned: id=%s op=%s ref=%s reason=%q",
		changeID, change.Op, change.ContainerRef, reason)
	return rec, nil
}
