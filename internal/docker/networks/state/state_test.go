...
func TestRequestChange_AppearsInRequested(t *testing.T) {
	...
	c := s.RequestChange(state.OpCreate, "my-net", nil)
	...
	if c.NetworkName != "my-net" {
		t.Errorf("networkName: got %q, want %q", c.NetworkName, "my-net")
	}
...
func TestRequestChange_MetaIsCopied(t *testing.T) {
	s := newState(t)
	meta := map[string]string{"driver": "bridge"}

	c := s.RequestChange(state.OpCreate, "net", meta)

	meta["driver"] = "MUTATED"
	if c.Meta["driver"] != "bridge" {
		t.Errorf("meta was not copied: got %q, want %q", c.Meta["driver"], "bridge")
	}
}
...
func TestRequestChange_IDs_AreUnique(t *testing.T) {
	...
	for i := range n {
		c := s.RequestChange(state.OpCreate, fmt.Sprintf("net-%d", i), nil)
...
func TestConfirmSuccess_MovesToActive(t *testing.T) {
	...
	rec, err := s.ConfirmSuccess(c.ID, "abc123")
	if err != nil {
	...
	if active[0].NetworkID != "abc123" {
		t.Errorf("NetworkID: got %q, want %q", active[0].NetworkID, "abc123")
	}
...
func TestConfirmSuccess_EmptyNetworkID_Allowed(t *testing.T) {
	// Remove and prune operations legitimately return no network ID.
	s := newState(t)
	c := s.RequestChange(state.OpRemove, "my-net", nil)
...
func TestConfirmSuccess_AlreadyConfirmed_NotFound(t *testing.T) {
	s := newState(t)
	c := s.RequestChange(state.OpCreate, "net", nil)
	_, _ = s.ConfirmSuccess(c.ID, "id1")

	_, err := s.ConfirmSuccess(c.ID, "id1")
	if !errors.Is(err, state.ErrChangeNotFound) {
...
func TestRecordFailure_MovesToFailed(t *testing.T) {
	s := newState(t)
	c := s.RequestChange(state.OpCreate, "bad-net", nil)
	opsErr := errors.New("network already exists")
...
func TestRecordFailure_NilError_UsesPlaceholder(t *testing.T) {
	s := newState(t)
	c := s.RequestChange(state.OpRemove, "net", nil)
...
func TestAbandon_MovesToAbandoned(t *testing.T) {
	s := newState(t)
	c := s.RequestChange(state.OpPrune, "", nil)
...
func TestSummary_ReflectsAllBuckets(t *testing.T) {
	...
	a := s.RequestChange(state.OpCreate, "net-a", nil)
	b := s.RequestChange(state.OpCreate, "net-b", nil)
	c := s.RequestChange(state.OpRemove, "net-c", nil)
	s.RequestChange(state.OpPrune, "", nil) // left in requested

	_, _ = s.ConfirmSuccess(a.ID, "id-a")
...
func TestFindByID_Active(t *testing.T) {
	s := newState(t)
	c := s.RequestChange(state.OpCreate, "net", nil)
	_, _ = s.ConfirmSuccess(c.ID, "net-id")
...
func TestFindByID_Failed(t *testing.T) {
	s := newState(t)
	c := s.RequestChange(state.OpRemove, "net", nil)
	_, _ = s.RecordFailure(c.ID, errors.New("no such network"))
...
func TestSnapshots_DoNotAliasInternalSlice(t *testing.T) {
	s := newState(t)
	s.RequestChange(state.OpCreate, "net", nil)
...
func TestConcurrentRequestsAndResolutions(t *testing.T) {
	...
	for i, id := range allIDs {
		...
		go func() {
			defer wg.Done()
			if i%2 == 0 {
				_, _ = s.ConfirmSuccess(id, "net-id")
			} else {
