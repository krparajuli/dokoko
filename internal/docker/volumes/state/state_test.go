...
func TestRequestChange_AppearsInRequested(t *testing.T) {
	...
	c := s.RequestChange(state.OpCreate, "my-vol", nil)
	...
	if c.VolumeName != "my-vol" {
		t.Errorf("volumeName: got %q, want %q", c.VolumeName, "my-vol")
	}
...
func TestRequestChange_MetaIsCopied(t *testing.T) {
	s := newState(t)
	meta := map[string]string{"driver": "local"}

	c := s.RequestChange(state.OpCreate, "vol", meta)

	meta["driver"] = "MUTATED"
	if c.Meta["driver"] != "local" {
		t.Errorf("meta was not copied: got %q, want %q", c.Meta["driver"], "local")
	}
}
...
func TestRequestChange_IDs_AreUnique(t *testing.T) {
	...
	for i := range n {
		c := s.RequestChange(state.OpCreate, fmt.Sprintf("vol-%d", i), nil)
...
func TestConfirmSuccess_MovesToActive(t *testing.T) {
	...
	rec, err := s.ConfirmSuccess(c.ID, "my-vol")
	if err != nil {
	...
	if active[0].VolumeName != "my-vol" {
		t.Errorf("VolumeName: got %q, want %q", active[0].VolumeName, "my-vol")
	}
...
func TestConfirmSuccess_EmptyVolumeName_Allowed(t *testing.T) {
	// Remove and prune operations legitimately return no volume name.
	s := newState(t)
	c := s.RequestChange(state.OpRemove, "my-vol", nil)
...
func TestConfirmSuccess_AlreadyConfirmed_NotFound(t *testing.T) {
	s := newState(t)
	c := s.RequestChange(state.OpCreate, "vol", nil)
	_, _ = s.ConfirmSuccess(c.ID, "vol")

	_, err := s.ConfirmSuccess(c.ID, "vol")
	if !errors.Is(err, state.ErrChangeNotFound) {
...
func TestRecordFailure_MovesToFailed(t *testing.T) {
	s := newState(t)
	c := s.RequestChange(state.OpCreate, "bad-vol", nil)
	opsErr := errors.New("volume already exists")
...
func TestRecordFailure_NilError_UsesPlaceholder(t *testing.T) {
	s := newState(t)
	c := s.RequestChange(state.OpRemove, "vol", nil)
...
func TestSummary_ReflectsAllBuckets(t *testing.T) {
	...
	a := s.RequestChange(state.OpCreate, "vol-a", nil)
	b := s.RequestChange(state.OpCreate, "vol-b", nil)
	c := s.RequestChange(state.OpRemove, "vol-c", nil)
	s.RequestChange(state.OpPrune, "", nil) // left in requested

	_, _ = s.ConfirmSuccess(a.ID, "vol-a")
...
func TestFindByID_Failed(t *testing.T) {
	s := newState(t)
	c := s.RequestChange(state.OpRemove, "vol", nil)
	_, _ = s.RecordFailure(c.ID, errors.New("no such volume"))
...
func TestSnapshots_DoNotAliasInternalSlice(t *testing.T) {
	s := newState(t)
	s.RequestChange(state.OpCreate, "vol", nil)
