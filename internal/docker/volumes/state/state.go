// Package dockervolumestate manages the in-memory lifecycle of Docker volume
...
// Operation is the kind of volume action being requested.
...
type StateChange struct {
	ID          string            // unique, sortable: "vchg-<nano>-<seq>"
	Op          Operation         // what to do
	VolumeName  string            // target volume name; empty for prune
	Meta        map[string]string // optional: driver, force flag, etc.
...
type ActiveRecord struct {
	Change      *StateChange
	VolumeName  string // volume name returned by the daemon (empty for remove/prune)
...
// State is the single source of truth for volume state changes in this process.
...
func New(log *logger.Logger) *State {
	log.LowTrace("initialising volume state store")
	s := &State{log: log}
	log.Debug("volume state store allocated (requested=0 active=0 failed=0 abandoned=0)")
	log.Info("volume state store ready")
	return s
}
...
func (s *State) RequestChange(op Operation, volumeName string, meta map[string]string) *StateChange {
	s.log.LowTrace("requesting state change: op=%s name=%s", op, volumeName)

	seq := atomic.AddUint64(&s.seq, 1)
	id := fmt.Sprintf("vchg-%d-%06d", time.Now().UnixNano(), seq)
	...
	s.log.Info("state change requested: id=%s op=%s name=%s", id, op, volumeName)
	return change
}

// ConfirmSuccess moves changeID from requested to active.  volumeName is the
// name returned by the daemon (may be empty for remove/prune operations).
//
// Returns ErrChangeNotFound if no requested change with that ID exists.
func (s *State) ConfirmSuccess(changeID, volumeName string) (*ActiveRecord, error) {
	s.log.LowTrace("confirming success: id=%s volumeName=%s", changeID, volumeName)
	...
	s.log.Debug("found change at requested[%d]: op=%s name=%s", idx, change.Op, change.VolumeName)
	...
	s.log.Info("state change confirmed active: id=%s op=%s name=%s resultName=%s",
		changeID, change.Op, change.VolumeName, volumeName)
	return rec, nil
}
...
func (s *State) RecordFailure(changeID string, err error) (*FailedRecord, error) {
	...
	s.log.Warn("state change failed: id=%s op=%s name=%s err=%s",
		changeID, change.Op, change.VolumeName, err)
	return rec, nil
}
...
func (s *State) Abandon(changeID, reason string) (*AbandonedRecord, error) {
	...
	s.log.Warn("state change abandoned: id=%s op=%s name=%s reason=%q",
		changeID, change.Op, change.VolumeName, reason)
	return rec, nil
}
...
var ErrChangeNotFound = errors.New("volume state change not found")
