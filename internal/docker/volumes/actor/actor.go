// Package dockervolumeactor dispatches Docker volume operations asynchronously,
// keeping dockervolumestate in sync with every outcome.
//
...
// opsProvider is the subset of *dockervolumeops.Ops that the Actor uses.
// Declared here so tests can inject a fake without a real Docker daemon.
type opsProvider interface {
	Create(ctx context.Context, opts dockervolume.CreateOptions) (dockervolume.Volume, error)
	List(ctx context.Context, opts dockervolume.ListOptions) (dockervolume.ListResponse, error)
	Inspect(ctx context.Context, name string) (dockervolume.Volume, error)
	Remove(ctx context.Context, name string, force bool) error
	Prune(ctx context.Context, pruneFilter dockerfilters.Args) (dockertypes.VolumesPruneReport, error)
}
...
type workItem struct {
	change *dockervolumestate.StateChange
	ctx    context.Context
	fn     func(ctx context.Context) (volumeName string, err error)
	done   chan struct{}
}
...
// callers that want to mutate or query volume state.
type Actor struct {
	ops   opsProvider
...
func New(ops opsProvider, st *dockervolumestate.State, log *logger.Logger, cfg *Config) *Actor {
	log.LowTrace("creating volume actor")
	c := cfg.withDefaults()
	...
	log.Info("volume actor ready (workers=%d queueSize=%d)", c.Workers, c.QueueSize)
	return a
}

// ── Mutating operations ───────────────────────────────────────────────────────

// Create submits an async volume creation.  State is updated to active (with
// the daemon-assigned name) on success, or failed on error.
func (a *Actor) Create(ctx context.Context, opts dockervolume.CreateOptions) (*Ticket, error) {
	a.log.LowTrace("submitting create: name=%q driver=%q", opts.Name, opts.Driver)

	meta := map[string]string{
		"name":   opts.Name,
		"driver": opts.Driver,
	}
	change := a.state.RequestChange(dockervolumestate.OpCreate, opts.Name, meta)
	a.log.Trace("create change registered: id=%s", change.ID)

	fn := func(ctx context.Context) (string, error) {
		a.log.Debug("worker: executing create for %q (change=%s)", opts.Name, change.ID)

		vol, err := a.ops.Create(ctx, opts)
		if err != nil {
			a.log.Error("worker: create failed for %q: %v", opts.Name, err)
			return "", err
		}

		a.log.Debug("worker: create %q succeeded → volumeName=%s", opts.Name, vol.Name)
		return vol.Name, nil
	}
...
func (a *Actor) Remove(ctx context.Context, name string, force bool) (*Ticket, error) {
	a.log.LowTrace("submitting remove: name=%q force=%v", name, force)

	meta := map[string]string{"force": fmt.Sprintf("%v", force)}
	change := a.state.RequestChange(dockervolumestate.OpRemove, name, meta)
	a.log.Trace("remove change registered: id=%s", change.ID)

	fn := func(ctx context.Context) (string, error) {
		a.log.Debug("worker: executing remove for %q (change=%s)", name, change.ID)

		if err := a.ops.Remove(ctx, name, force); err != nil {
			a.log.Error("worker: remove failed for %q: %v", name, err)
			return "", err
		}

		a.log.Debug("worker: remove %q succeeded", name)
		return "", nil
	}
...
func (a *Actor) Prune(ctx context.Context, pruneFilter dockerfilters.Args) (*Ticket, error) {
	...
	change := a.state.RequestChange(dockervolumestate.OpPrune, "", meta)
	...
	fn := func(ctx context.Context) (string, error) {
		...
		a.log.Debug("worker: prune succeeded: removed=%v spaceReclaimed=%d",
			report.VolumesDeleted, report.SpaceReclaimed)
		return "", nil
	}
...
// List asynchronously lists volumes and sends the result on the returned
// buffered channel.  The caller may ignore the channel; the goroutine will
// not leak.
func (a *Actor) List(ctx context.Context, opts dockervolume.ListOptions) <-chan ListResult {
	a.log.LowTrace("dispatching async list")
	ch := make(chan ListResult, 1)

	go func() {
		a.log.Trace("async list: calling ops.List")
		resp, err := a.ops.List(ctx, opts)
		if err != nil {
			a.log.Error("async list: ops.List failed: %v", err)
		} else {
			a.log.Debug("async list: returned %d volumes", len(resp.Volumes))
		}
		ch <- ListResult{Response: resp, Err: err}
	}()

	return ch
}

// Inspect asynchronously inspects the named volume and sends the result on the
// returned buffered channel.
func (a *Actor) Inspect(ctx context.Context, name string) <-chan InspectResult {
	a.log.LowTrace("dispatching async inspect: %s", name)
	ch := make(chan InspectResult, 1)

	go func() {
		a.log.Trace("async inspect: calling ops.Inspect for %q", name)
		vol, err := a.ops.Inspect(ctx, name)
		if err != nil {
			a.log.Error("async inspect: ops.Inspect failed for %q: %v", name, err)
		} else {
			a.log.Debug("async inspect: %q driver=%s mountpoint=%s", name, vol.Driver, vol.Mountpoint)
		}
		ch <- InspectResult{Volume: vol, Err: err}
	}()
...
func (a *Actor) Close() {
	a.closeOnce.Do(func() {
		a.log.LowTrace("closing volume actor")
		a.log.Debug("signalling workers to stop and draining queue")
		close(a.closed)
		a.wg.Wait()
		a.log.Info("volume actor closed, all workers exited")
	})
}

// ── Internal helpers ──────────────────────────────────────────────────────────

func (a *Actor) submit(change *dockervolumestate.StateChange, ctx context.Context, fn func(context.Context) (string, error)) (*Ticket, error) {
	a.log.Trace("submit: change=%s name=%s", change.ID, change.VolumeName)
	...
	default:
		a.log.Warn("submit: queue full, abandoning change %s (op=%s name=%s)",
			change.ID, change.Op, change.VolumeName)
	...
	for {
		...
		case item, ok := <-a.queue:
			...
			a.log.Trace("worker %d: picked up change %s (op=%s name=%s)",
				workerID, item.change.ID, item.change.Op, item.change.VolumeName)
...
func (a *Actor) execute(workerID int, item workItem) {
	...
	a.log.Debug("worker %d: executing change %s (op=%s name=%s)",
		workerID, changeID, item.change.Op, item.change.VolumeName)
	...
	volumeName, err := item.fn(item.ctx)
	if err != nil {
	...
	a.log.Debug("worker %d: change %s succeeded (volumeName=%q)", workerID, changeID, volumeName)
	_, _ = a.state.ConfirmSuccess(changeID, volumeName)
